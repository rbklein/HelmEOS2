...
def attraction_func(T):
    ...
    """
    return a_PR * (1 + kappa * (1 - sqrt(T / T_c)))**2
...
def Peng_Robinson(rho, T):
    ...
    density_term = (1 + (1+sqrt(2)) * b_PR * rho) / (1 + (1-sqrt(2)) * b_PR * rho)
...
_drpdT_root = grad(root_func_pressure, 0)
...
grad_attraction = grad(attraction_func)
def _root_func_energy(T, rho, e):
    ...
    return e - molecular_dofs / 2 * R_specific * T - num / (2 * sqrt(2) * b_PR) * log(density_term)
_dredT_root = grad(_root_func_energy, 0)
