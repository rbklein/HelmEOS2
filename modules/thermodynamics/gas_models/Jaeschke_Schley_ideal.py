...
_b0 = 2.500020000 + 1.0
_b13 = jnp.array([2.044520000, 2.033660000])
_theta13 = jnp.array([3.022758166, 1.589964364]) * T_c
_b24 = jnp.array([-1.060440000, 0.013930000])
_theta24 = jnp.array([-2.844425476, 1.121596090]) * T_c

# reference values mentioned in KW-article
T_ref = 298.15      # K
p_ref = 0.101325e6  # Pa
rho_ref = p_ref / (R_specific * T_ref) # kg m^-3 CHECK DIMENSIONALITY
...
def enthalpy_integral(T):
    '''
        ...
        The quantity is divided by the molar mass to obtain per mass units
    '''
    linear_term = _b0 * T
    tanh_term = 0.0
    for i in range(2):
    ...
    return R_specific * (linear_term + tanh_term + coth_term) #/ MOLAR_MASS
...
def entropy_integral(T):
    ...
    return R_specific * (ln_term + sinh_term + cosh_term) #/ MOLAR_MASS
