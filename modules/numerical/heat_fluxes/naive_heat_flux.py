...
from modules.thermodynamics.constitutive    import thermal_conductivity
from jax.numpy                              import stack, zeros_like, zeros
...
def div_x_naive_heat_flux_2d(u, T):
    ...
    f_m1_x = zeros((n_x + 1, n_y))
    f_m2_x = zeros((n_x + 1, n_y))
    f_E_x = q
...
def div_y_naive_heat_flux_2d(u, T):
    ...
    f_m1_y = zeros((n_x, n_y + 1))
    f_m2_y = zeros((n_x, n_y + 1))
    f_E_y = q
...
def div_naive_heat_flux_1d(u, T):
    ...
    F = stack((f_rho_x, f_m_x, f_E_x), axis = 0)
    return F[:, 1:] - F[:, :-1]