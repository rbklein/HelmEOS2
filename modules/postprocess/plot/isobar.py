...
from modules.thermodynamics.EOS                         import temperature_rpt, density_ptr
...
from jax.numpy                                          import linspace, ones_like

def isobar_rho(p_iso, rho1, rho2, num_points = 100):
    rho = pad_1d_to_mesh(linspace(rho1, rho2, num_points))
    p = p_iso * ones_like(rho)
...
def isobar_T(p_iso, T1, T2, num_points = 100):
    T = pad_1d_to_mesh(linspace(T1, T2, num_points))
    p = p_iso * ones_like(T)
